...
struct FlavorPickerView: View {
    ...
    private var sectionHeader: some View {
        HStack {
            ...
            Text(viewModel.flavorsLocked
                 ? "\(viewModel.selectedFlavors.count) locked"
        ...
        return VStack(spacing: 8) {
            ...
            if !viewModel.selectedFlavors.isEmpty {
                Button {
                    ...
                    Label("Lock Blend", systemImage: "lock.fill")
    ...
    private func lockedView(viewModel: BatchConfigViewModel) -> some View {
        ...
        let terpenes = Array(viewModel.selectedFlavors.keys.filter {
            if case .terpene = $0 { return true }
            return false
        })
        let oils = Array(viewModel.selectedFlavors.keys.filter {
            if case .oil = $0 { return true }
            return false
        })
...
//        }
        ...
        if !viewModel.flavorCompositionLocked {
            ...
            let canLock = terpenesReady && oilsReady

            Button {
                viewModel.lockComposition()
            } label: {
            ...
            if !canLock {
                Text("Both blends must sum to 100% to lock")
                    .font(.caption)
                    .foregroundStyle(.red)
                    .padding(.horizontal, 16)
                    .padding(.top, 4)
