...
//    • Residue losses (beaker + syringe)
//    • Active compound loss and per-gummy dose
//    • Gummy mixture density (mass / volume)
//    • Preservative mass fractions (potassium sorbate, citric acid)
//    • Recommended overage factor for the next batch
//  Each result row shows the value with its significant-figures count and
//  offers one-tap density/overage application to SystemConfig.
//
...
struct MeasurementCalculationsView: View {
    ...
    private var gold: Color { systemConfig.designTertiaryAccent }
    ...
    private var overageErrorRow: some View {
        if let err = overageError {
            let errPct = err * 100.0
            let isPositive = errPct >= 0
            let color: Color = isPositive ? Self.softGreen : CMTheme.lockRed
            HStack(spacing: 6) {
                Text("Overage Error")
                    .cmRowLabel()
                Spacer()
                Text(String(format: "%+.2f%%", errPct))
                    .cmMono12()
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Button {
                ...
                HStack {
                    Text("Experiment Data").cmSectionTitle(accent: systemConfig.accent)
                    Spacer()
            ...
            calcRow("Average Gummy Active Dose",     value: averageGummyActiveDose,  unit: viewModel.units.rawValue, valueColor: gold)
