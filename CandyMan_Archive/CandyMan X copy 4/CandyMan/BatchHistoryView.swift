                    ...
                    HStack(spacing: 16) {
                        ...
                        NavigationLink(destination: TrashView()) {
                            Image(systemName: "trash")
                                .font(.system(size: 15, weight: .medium))
                                .foregroundStyle(trashedBatches.isEmpty ? CMTheme.textTertiary : CMTheme.lockRed)
                        }
                    }
                }
            }
...
struct BatchDetailView: View {
    ...
    private var activesSection: some View {
        let gold = systemConfig.designTertiaryAccent
                            ...
                            HStack(spacing: 6) {
                                if !batch.batchID.isEmpty {
                                    Text(batch.batchID)
                                        .font(.caption).fontWeight(.bold)
                                        .padding(.horizontal, 6).padding(.vertical, 2)
                                        .background(CMTheme.lockRed.opacity(0.15))
                                        .foregroundStyle(CMTheme.lockRed)
                                        .cornerRadius(4)
                                }
