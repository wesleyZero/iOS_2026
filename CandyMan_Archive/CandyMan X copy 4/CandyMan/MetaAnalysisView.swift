...
struct MetaAnalysisView: View {
    ...
    private var summaryCard: some View {
        ...
        return VStack(spacing: 0) {
            HStack {
                Text("Final Mixture Mass vs Volume").cmSectionTitle(accent: systemConfig.accent)
                Spacer()
                Button {
                    CMHaptic.light()
    ...
    private var regressionChart: some View {
        ...
        return AnyView(
            VStack(spacing: 0) {
                HStack {
                    Text("Mass vs Volume — Final Gummy Mixture").cmSectionTitle(accent: systemConfig.accent)
                    Spacer()
                }
                .padding(.horizontal, 16).padding(.vertical, 12)
    ...
    private var regressionStatsCard: some View {
        ...
        return AnyView(
            VStack(spacing: 0) {
                HStack {
                    Text("Regression Statistics").cmSectionTitle(accent: systemConfig.accent)
                    Spacer()
                }
                .padding(.horizontal, 16).padding(.vertical, 12)
    ...
    private var residualsCard: some View {
        ...
        return AnyView(
            VStack(spacing: 0) {
                HStack {
                    Text("Residuals").cmSectionTitle(accent: systemConfig.accent)
                    Spacer()
                }
                .padding(.horizontal, 16).padding(.vertical, 12)
                ...
                Chart {
                    ...
                    ForEach(residuals) { r in
                        ...
                        .foregroundStyle(r.residual >= 0 ? systemConfig.accent : CMTheme.lockRed)
                        .cornerRadius(2)
                    }
                }
                .chartXAxisLabel("Syringe Volume (mL)", alignment: .center)
    ...
    private var dataTableCard: some View {
        ...
        return VStack(spacing: 0) {
            HStack {
                Text("Batch Data").cmSectionTitle(accent: systemConfig.accent)
                Spacer()
            }
            .padding(.horizontal, 16).padding(.vertical, 12)
                    ...
                    HStack(spacing: 4) {
                        ...
                        if let r = residual {
                            Text(String(format: "%+.3f", r))
                                .cmMono11()
                                .foregroundStyle(abs(r) < 0.5 ? CMTheme.success : (abs(r) < 1.5 ? Color(red: 0.95, green: 0.75, blue: 0.30) : CMTheme.lockRed))
                                .frame(width: 60, alignment: .trailing)
                        }
                    }
