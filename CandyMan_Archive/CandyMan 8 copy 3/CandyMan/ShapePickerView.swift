...
struct ShapePickerView: View {
        ...
        return VStack(spacing: 4) {
            HStack {
                ...
                if viewModel.trayCount != 1 {
                    Button {
                        CMHaptic.light()
                        withAnimation(.cmSpring) { viewModel.trayCount = 1 }
                    } label: {
                ...
                Text("\(viewModel.trayCount)")
                    .font(.subheadline).fontWeight(.semibold)
                    .foregroundStyle(viewModel.trayCount > 1 ? Color(red: 0.929, green: 0.278, blue: 0.290) : CMTheme.textTertiary)
            }
    ...
    private func shapeButton(for shape: GummyShape) -> some View {
        ...
        return Button {
            ...
            .overlay {
                if sel {
                    ShapeButtonSwirlOverlay(sfSymbol: shape.sfSymbol, name: shape.rawValue)
                }
            }
            ...
            .background(
                RoundedRectangle(cornerRadius: CMTheme.buttonRadius, style: .continuous)
                    .fill(sel ? CMTheme.fieldBG : CMTheme.fieldBG)
            )
            .overlay(
                RoundedRectangle(cornerRadius: CMTheme.buttonRadius, style: .continuous)
...
/// with a rotating AngularGradient (SkittleSwirl) as the foreground.
/// Drawn on top so the dark base text is hidden and only the gradient shows.
