...
struct ShapePickerView: View {
        ...
        return VStack(spacing: 4) {
            ...
            HStack(alignment: .firstTextBaseline) {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 6) {
                    TextField("0.0", value: gelatinUseMass ? massBinding : $viewModel.gelatinPercentage,
                              format: .number.precision(.fractionLength(1...3)))
                        .keyboardType(.decimalPad).multilineTextAlignment(.center)
                        .font(.system(size: 30, weight: .bold)).skittleSwirlWide().fixedSize().selectAllOnFocus()
    ...
    private var chooseShape: some View {
        VStack(spacing: 0) {
            HStack {
                Text("Shape").font(.headline).foregroundStyle(CMTheme.textPrimary)
    ...
    private func chooseTrays(viewModel: BatchConfigViewModel) -> some View {
        ...
        return VStack(spacing: 4) {
            // Header
            HStack {
                Text("Trays").font(.headline).foregroundStyle(CMTheme.textPrimary)
                if traysChanged {
                    Button {
                        CMHaptic.light()
                        withAnimation(.cmSpring) {
    ...
    private func chooseActive(viewModel: BatchConfigViewModel) -> some View {
        ...
        return VStack(spacing: 8) {
                ...
                ForEach(Active.allCases) { substance in
                    ...
                    Button {
                        ...
                        Text(substance.rawValue)
                            ...
                            .background(
                                RoundedRectangle(cornerRadius: CMTheme.chipRadius, style: .continuous)
                                    .fill(isSelected ? CMTheme.chipBG : Color.clear)
                            )
                            .if(isSelected) { $0.skittleSwirlWide() }
                            .if(!isSelected) { $0.foregroundStyle(CMTheme.textSecondary) }
                    }
                    .buttonStyle(.plain)
                }
            }
            .padding(4)
            .background(
                RoundedRectangle(cornerRadius: CMTheme.chipRadius + 2, style: .continuous)
...
/// **SkittleSwirl** — Animated "CandyMan" title with a rotating angular gradient
