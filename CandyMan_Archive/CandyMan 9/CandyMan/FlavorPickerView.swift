        ...
        return VStack(spacing: 0) {
            HStack(alignment: .firstTextBaseline) {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 6) {
                    TextField("0.0", value: oilUseMass ? massBinding : $viewModel.flavorOilVolumePercent,
    ...
    private func terpeneBlendView(viewModel: BatchConfigViewModel) -> some View {
        ...
        return VStack(spacing: 0) {
            HStack(alignment: .firstTextBaseline) {
                ...
                Spacer()
                HStack(alignment: .firstTextBaseline, spacing: 6) {
                    TextField("0.0", value: activeBinding,
                              format: .number.precision(.fractionLength(fractionRange)))
                        .keyboardType(.decimalPad).multilineTextAlignment(.center)
                        .font(.system(size: 30, weight: .bold)).skittleSwirlWide().fixedSize().selectAllOnFocus()
                    HStack(spacing: 4) {
                        Text(unitPrefix).font(.system(size: 20)).foregroundStyle(CMTheme.textSecondary)
                        Text(unitSuffix).font(.system(size: 20)).foregroundStyle(goldenYellow)
                    }
                }
                Spacer()
                Button {
