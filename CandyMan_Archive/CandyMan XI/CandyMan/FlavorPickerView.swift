...
struct FlavorOilPickerView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 8) {
            // Header
            HStack {
                Text("Flavor Oils").font(.headline).foregroundStyle(systemConfig.accent)
                if oilsChanged {
                    Button {
                        CMHaptic.light()
    ...
    private func oilPickingView(viewModel: BatchConfigViewModel) -> some View {
        ...
        return VStack(spacing: 8) {
            ...
            if !selectedOils.isEmpty && !viewModel.oilsLocked {
                Button {
                    CMHaptic.medium()
                    // Set default max slider cap = 100 * (1 / count) * 1.5, rounded to nearest 5
                    let count = Double(selectedOils.count)
                    let rawCap = (100.0 / count) * 1.5
                    let roundedCap = min((rawCap / 5.0).rounded() * 5.0, 100.0)
                    maxSliderPercent = roundedCap
        ...
        return VStack(spacing: 0) {
            HStack(alignment: .firstTextBaseline) {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 6) {
                    NumericField(value: oilUseMass ? massBinding : $viewModel.flavorOilVolumePercent, decimals: 3)
                    ...
                    Slider(
                        value: Binding(
                            ...
                            set: { newValue in
                                ...
                                // Clamp: stop slider if progress bar hits 100%
                        ...
                        in: 0...maxSliderPercent, step: 5
                    )
                    .tint(CMTheme.textPrimary)
                    .overlay(
                        GeometryReader { geo in
                            let fraction = maxSliderPercent > 0 ? currentValue / maxSliderPercent : 0
                            let thumbX = geo.size.width * fraction
                            Text("\(Int(currentValue))%")
                                .font(.system(size: 9, weight: .bold, design: .rounded))
                                .foregroundStyle(CMTheme.textPrimary)
                                .position(x: thumbX, y: geo.size.height / 2 - 20)
...
struct TerpenePickerView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 8) {
            // Header
            HStack {
                Text("Terpenes").font(.headline).foregroundStyle(systemConfig.accent)
                if terpenesChanged {
                    Button {
                        CMHaptic.light()
                        withAnimation(.cmSpring) {
        ...
        return VStack(spacing: 8) {
            ...
            if !selectedTerpenes.isEmpty && !viewModel.terpenesLocked {
                Button {
                    CMHaptic.medium()
                    // Set default max slider cap = 100 * (1 / count) * 1.5, rounded to nearest 5
                    let count = Double(selectedTerpenes.count)
                    let rawCap = (100.0 / count) * 1.5
                    let roundedCap = min((rawCap / 5.0).rounded() * 5.0, 100.0)
                    maxSliderPercent = roundedCap
    ...
    private func terpeneBlendView(viewModel: BatchConfigViewModel) -> some View {
        ...
        let goldenYellow = Color(red: 0.85, green: 0.65, blue: 0.0)
        ...
        return VStack(spacing: 0) {
            HStack(alignment: .firstTextBaseline) {
                ...
                Spacer()
                HStack(alignment: .firstTextBaseline, spacing: 6) {
                    NumericField(value: activeBinding, decimals: terpUsePercent ? 4 : 1)
                        .multilineTextAlignment(.center)
                        .font(.system(size: 30, weight: .bold)).skittleSwirlWide().fixedSize()
                    HStack(spacing: 4) {
                        Text(unitPrefix).font(.system(size: 20)).foregroundStyle(goldenYellow)
                        Text(unitSuffix).font(.system(size: 20)).foregroundStyle(Color(white: 0.35))
                    }
                }
                Spacer()
                Button {
                    ...
                    Slider(
                        value: Binding(
                            ...
                            set: { newValue in
                                ...
                                if clamped != oldValue && systemConfig.sliderVibrationsEnabled {
                                    let generator = UIImpactFeedbackGenerator(style: .light)
                                    generator.impactOccurred(intensity: clamped / 100.0)
                                }
                                #endif
                            }
                        ),
