        ...
        return VStack(spacing: 0) {
            ...
            ForEach(GummyShape.allCases) { shape in
                HStack {
                    ...
                    if !systemConfig.moldVolumeIsDefault(for: shape) {
                        CMResetButton { systemConfig.resetMoldVolume(for: shape) }
                    }
                    Spacer()
            ...
            ForEach(GummyShape.allCases) { shape in
                HStack {
                    ...
                    if !systemConfig.moldCountIsDefault(for: shape) {
                        CMResetButton { systemConfig.resetMoldCount(for: shape) }
                    }
                    Spacer()
            ...
            NavigationLink {
                ...
                HStack {
                    ...
                    Text("Calibrate a Mold").font(.body).foregroundStyle(CMTheme.strawberryRed)
                    Spacer()
        ...
        return VStack(spacing: 0) {
            ...
            HStack {
                ...
                if systemConfig.glucoseToSugarMassRatio != 1.000 {
                    CMResetButton { systemConfig.glucoseToSugarMassRatio = 1.000 }
                }
                Spacer()
        ...
        return VStack(spacing: 0) {
            ...
            HStack {
                ...
                if systemConfig.waterToGelatinMassRatio != 3.000 {
                    CMResetButton { systemConfig.waterToGelatinMassRatio = 3.000 }
                }
                Spacer()
            ...
            HStack {
                ...
                if systemConfig.waterMassPercentInSugarMix != 17.34 {
                    CMResetButton { systemConfig.waterMassPercentInSugarMix = 17.34 }
                }
                Spacer()
            ...
            if useMass {
                ...
                HStack {
                    ...
                    if systemConfig.potassiumSorbatePercent != 0.078 {
                        CMResetButton { systemConfig.potassiumSorbatePercent = 0.078 }
                    }
                    Spacer()
                ...
                HStack {
                    ...
                    if systemConfig.citricAcidPercent != 0.638 {
                        CMResetButton { systemConfig.citricAcidPercent = 0.638 }
                    }
                    Spacer()
        ...
        return VStack(spacing: 0) {
            ...
            HStack {
                ...
                if viewModel.overageFactor != 1.03 {
                    CMResetButton { viewModel.overageFactor = 1.03 }
                }
                Spacer()
            ...
            ForEach(Array(rows.enumerated()), id: \.offset) { index, row in
                ...
                HStack {
                    ...
                    if !systemConfig.densityIsDefault(keyPath) {
                        CMResetButton { systemConfig.resetDensity(keyPath) }
                    }
                    Spacer()
    ...
    private func settingsActionsSection(systemConfig: SystemConfig) -> some View {
        ...
        return VStack(spacing: 0) {
            Button {
                ...
                .foregroundStyle(hasChanges ? CMTheme.strawberryRed : CMTheme.textTertiary)
                .padding(.horizontal, 16).padding(.vertical, 12)
            }
            .buttonStyle(.plain)
            .disabled(!hasChanges)
