                        ...
                        ForEach(activeBatches) { batch in
                            ...
                            .swipeActions(edge: .trailing, allowsFullSwipe: true) {
                                Button(role: .destructive) {
                                    CMHaptic.medium()
                                    withAnimation {
                                        batch.isTrashed = true
                                        batch.trashedDate = .now
                                    }
                                } label: {
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    NavigationLink(destination: TrashView()) {
                        Image(systemName: "trash")
                            ...
                            .foregroundStyle(trashedBatches.isEmpty ? CMTheme.textTertiary : CMTheme.strawberryRed)
                    }
                }
            }
            #endif
...
struct BatchDetailView: View {
    ...
    private var batchOutputCard: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                GlassCopyButton { batchDetailCopyJSON(batchOutputJSON(), copiedConfirmation: $copiedConfirmation) }
                    .padding(.leading, 6)
            }
    ...
    private var headerSection: some View {
        VStack(spacing: 0) {
            HStack {
                Text("Batch Info").font(.headline).foregroundStyle(CMTheme.textPrimary)
                Spacer()
                GlassCopyButton { batchDetailCopyJSON(headerJSON(), copiedConfirmation: $copiedConfirmation) }
            }
    ...
    private var userInputsSection: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                Spacer()
                GlassCopyButton { batchDetailCopyJSON(userInputsJSON(), copiedConfirmation: $copiedConfirmation) }
            }
    ...
    private func buildJSONString() -> String {
        ...
        // DehydrationTracking
        let dehydrationArr: [[String: Any]] = dryWeightEntries.map { e in
            var d: [String: Any] = [
                            ...
                            HStack(spacing: 6) {
                                if !batch.batchID.isEmpty {
                                    Text(batch.batchID)
                                        .font(.caption).fontWeight(.bold)
                                        .padding(.horizontal, 6).padding(.vertical, 2)
                                        .background(CMTheme.strawberryRed.opacity(0.15))
