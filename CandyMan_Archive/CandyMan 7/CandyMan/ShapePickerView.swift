...
struct ShapePickerView: View {
    ...
    private var canCalculate: Bool { viewModel.canCalculateBatch }
        ...
        VStack(spacing: 8) {
            HStack {
                ...
                if viewModel.activeTemplateID != nil {
                    HStack(spacing: 6) {
                        Text("Info \(Text("saved").foregroundColor(CMTheme.strawberryRed)) as \(Text(viewModel.activeTemplateName).foregroundColor(CMTheme.strawberryRed))")
                            .font(.caption)
                    ...
                    Button {
                        ...
                        HStack(spacing: 6) {
                            Text("Info \(canCalculate ? Text("not saved").foregroundColor(CMTheme.strawberryRed) : Text("not saved")) as template")
                                .font(.caption)
                                .foregroundStyle(CMTheme.textTertiary)
    ...
    private var templateSection: some View {
        VStack(spacing: 8) {
            ...
            if viewModel.activeTemplateID != nil {
                Text("The \(Text(viewModel.activeTemplateName).foregroundColor(CMTheme.strawberryRed)) template is being used.")
                    .font(.caption)
                    .foregroundStyle(CMTheme.textTertiary)
                    .padding(.horizontal, 16)
                    .padding(.bottom, 10)
