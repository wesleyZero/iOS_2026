...
enum CMTheme {
    ...
    static let strawberryRed = Color(red: 0.929, green: 0.278, blue: 0.290)

    // MARK: Thresholds

    static let blendSumTolerance: Double = 0.5
    static let densityComparisonTolerance: Double = 0.0001
    static let toastDismissDelay: Double = 3.0
...
// MARK: - Haptic Slider Binding

/// Creates a Binding that triggers light haptic feedback proportional to the value on change.
func hapticSliderBinding(
    get: @escaping () -> Double,
    ...
    Binding(
        ...
        set: { newValue in
            ...
            set(newValue)
            #if canImport(UIKit)
            if newValue != oldValue && vibrationsEnabled {
                UIImpactFeedbackGenerator(style: .light).impactOccurred(intensity: newValue / 100.0)
            }
            #endif
        }
    )
...
// MARK: - Inline Reset Button

struct CMResetButton: View {
    let action: () -> Void
