...
// MARK: - Flavor Selection Partitioning

extension FlavorSelection {
    var isTerpene: Bool {
        if case .terpene = self { return true }
        return false
    }

    var isOil: Bool {
        if case .oil = self { return true }
        return false
    }
}

extension Dictionary where Key == FlavorSelection, Value == Double {
    var terpeneEntries: [Key: Value] {
        filter { $0.key.isTerpene }
    }

    var oilEntries: [Key: Value] {
        filter { $0.key.isOil }
    }

    var terpeneBlendTotal: Double {
        terpeneEntries.values.reduce(0, +)
    }

    var oilBlendTotal: Double {
        oilEntries.values.reduce(0, +)
    }
}
