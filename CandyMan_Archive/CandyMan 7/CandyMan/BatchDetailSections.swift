...
struct BatchQuantitativeDataSection: View {
    ...
    private func quantitativeDataJSON() -> [String: Any] {
        ...
        let gelatinItems    = sortedComponents.filter { $0.group == "Gelatin Mix" }
        let sugarItems      = sortedComponents.filter { $0.group == "Sugar Mix" }
        let activeMass   = activationItems.reduce(0.0) { $0 + $1.mass_g }
        let activeVolume = activationItems.reduce(0.0) { $0 + $1.volume_mL }
        let gelatinMass  = gelatinItems.reduce(0.0) { $0 + $1.mass_g }
        let gelatinVolume = gelatinItems.reduce(0.0) { $0 + $1.volume_mL }
        let sugarMass    = sugarItems.reduce(0.0) { $0 + $1.mass_g }
        let sugarVolume  = sugarItems.reduce(0.0) { $0 + $1.volume_mL }
        let finalMixMass = activeMass + gelatinMass + sugarMass
        let finalMixVolume = activeVolume + gelatinVolume + sugarVolume
        let overageFactor = batch.vBase_mL > 0 ? batch.vMix_mL / batch.vBase_mL : 1.0
        let targetVolume = batch.vBase_mL
        let finalMixVolumeNoOverage = overageFactor > 0 ? finalMixVolume / overageFactor : finalMixVolume
        let quantifiedError = finalMixVolumeNoOverage - targetVolume
        let relativeError = targetVolume > 0 ? (quantifiedError / targetVolume) * 100.0 : 0.0
        return [
            "targetVolumes": ["volumePerMold_mL": batch.wellCount > 0 ? targetVolume / Double(batch.wellCount) : 0, "volumePerTray_mL": batch.trayCount > 0 ? targetVolume / Double(batch.trayCount) : 0, "totalTargetVolume_mL": targetVolume],
            "mixTotals": ["activationMix": ["mass_g": activeMass, "volume_mL": activeVolume], "gelatinMix": ["mass_g": gelatinMass, "volume_mL": gelatinVolume], "sugarMix": ["mass_g": sugarMass, "volume_mL": sugarVolume]],
            "finalMix": ["withOverage": ["mass_g": finalMixMass, "volume_mL": finalMixVolume], "withoutOverage": ["mass_g": finalMixMass / overageFactor, "volume_mL": finalMixVolumeNoOverage]],
            "error": ["quantifiedError_mL": quantifiedError, "relativeErrorPct": relativeError],
        ] as [String: Any]
    }
}
...
struct BatchRelativeDataSection: View {
    ...
    private func relativeDataJSON() -> [String: Any] {
        ...
        let finalMixMass = activeMass + gelatinMass + sugarMass
        let finalMixVolume = activeVolume + gelatinVolume + sugarVolume
        func percentOf(_ part: Double, _ whole: Double) -> Double { whole > 0 ? (part / whole) * 100.0 : 0 }
        return [
            "components": sortedComponents.map { ["label": $0.label, "group": $0.group, "massPct": percentOf($0.mass_g, finalMixMass), "volumePct": percentOf($0.volume_mL, finalMixVolume)] as [String: Any] },
            "mixTotals": ["activationMix": ["massPct": percentOf(activeMass, finalMixMass), "volumePct": percentOf(activeVolume, finalMixVolume)], "gelatinMix": ["massPct": percentOf(gelatinMass, finalMixMass), "volumePct": percentOf(gelatinVolume, finalMixVolume)], "sugarMix": ["massPct": percentOf(sugarMass, finalMixMass), "volumePct": percentOf(sugarVolume, finalMixVolume)]],
            "goopRatio": ["mass": gelatinMass > 0 ? sugarMass / gelatinMass : 0, "volume": gelatinVolume > 0 ? sugarVolume / gelatinVolume : 0],
        ] as [String: Any]
    }
}
            ...
            HStack {
                ...
                Button {
                    ...
                    Image(systemName: batch.dehydrationLocked ? "lock.fill" : "lock.open.fill")
                        ...
                        .foregroundStyle(batch.dehydrationLocked ? CMTheme.strawberryRed : CMTheme.textTertiary)
                }
                .buttonStyle(.plain)
                Spacer()
            ...
            HStack {
                ...
                Button {
                    ...
                    Image(systemName: batch.notesLocked ? "lock.fill" : "lock.open.fill")
                        .font(.system(size: 14, weight: .medium))
                        .foregroundStyle(batch.notesLocked ? CMTheme.strawberryRed : CMTheme.textTertiary)
                }
                .buttonStyle(.plain)
                Spacer()
