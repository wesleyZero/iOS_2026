...
func linearRegression(xs: [Double], ys: [Double]) -> RegressionResult? {
...
struct MetaAnalysisView: View {
    ...
    private var summaryCard: some View {
        ...
        return VStack(spacing: 0) {
            HStack {
                Text("Final Mixture Mass vs Volume").font(.headline).foregroundStyle(systemConfig.accent)
                Spacer()
                Button {
                    CMHaptic.light()
    ...
    private var regressionChart: some View {
        ...
        return AnyView(
            VStack(spacing: 0) {
                HStack {
                    Text("Mass vs Volume — Final Gummy Mixture").font(.headline).foregroundStyle(systemConfig.accent)
                    Spacer()
                }
                .padding(.horizontal, 16).padding(.vertical, 12)
                ...
                Chart {
                    ...
                    ForEach(bandPoints) { bp in
                        ...
                        .foregroundStyle(systemConfig.accent.opacity(0.1))
                    }
                    ...
                    ForEach(Array(lineXs.enumerated()), id: \.offset) { _, x in
                        ...
                        .foregroundStyle(systemConfig.accent.opacity(0.6))
    ...
    private var regressionStatsCard: some View {
        ...
        return AnyView(
            VStack(spacing: 0) {
                HStack {
                    Text("Regression Statistics").font(.headline).foregroundStyle(systemConfig.accent)
                    Spacer()
                }
                .padding(.horizontal, 16).padding(.vertical, 12)
    ...
    private var residualsCard: some View {
        ...
        return AnyView(
            VStack(spacing: 0) {
                HStack {
                    Text("Residuals").font(.headline).foregroundStyle(systemConfig.accent)
                    Spacer()
                }
                .padding(.horizontal, 16).padding(.vertical, 12)
                ...
                Chart {
                    ...
                    ForEach(residuals) { r in
                        ...
                        .foregroundStyle(r.residual >= 0 ? systemConfig.accent : Color(red: 0.929, green: 0.278, blue: 0.290))
                        .cornerRadius(2)
                    }
                }
                .chartXAxisLabel("Syringe Volume (mL)", alignment: .center)
    ...
    private var dataTableCard: some View {
        ...
        return VStack(spacing: 0) {
            HStack {
                Text("Batch Data").font(.headline).foregroundStyle(systemConfig.accent)
                Spacer()
            }
            .padding(.horizontal, 16).padding(.vertical, 12)
                    ...
                    HStack(spacing: 4) {
                        ...
                        if let r = residual {
                            Text(String(format: "%+.3f", r))
                                .font(.system(size: 11, design: .monospaced))
                                .foregroundStyle(abs(r) < 0.5 ? CMTheme.success : (abs(r) < 1.5 ? Color(red: 0.95, green: 0.75, blue: 0.30) : Color(red: 0.929, green: 0.278, blue: 0.290)))
                                .frame(width: 60, alignment: .trailing)
                        }
                    }
    ...
    private func statRow(_ label: String, value: String, highlight: Bool = false) -> some View {
        HStack(spacing: 6) {
            ...
            Text(value)
                .font(.system(size: 12, design: .monospaced))
                .foregroundStyle(highlight ? systemConfig.accent : CMTheme.textSecondary)
        }
        .padding(.horizontal, 16).padding(.vertical, 3)
    }
                                    ...
                                    if let r = residual {
                                        Text(String(format: "%+.3f", r))
                                            ...
                                            .foregroundStyle(
                                                ...
                                                : abs(r) < 1.5 ? Color(red: 0.95, green: 0.75, blue: 0.30)
