    ...
    private var activesSection: some View {
        ...
        return VStack(spacing: 0) {
            ...
            if viewModel.selectedActive == .LSD {
                ...
                Group {
                    Text("Dissolve 1 Tab in \(String(format: "%.0f", transferWater)) mL of pure distilled water, let it dissolve, then transfer ")
                        .foregroundStyle(CMTheme.textTertiary)
                    + Text(String(format: "%.0f mL", keptVolume))
                        .foregroundStyle(gold)
                    + Text(" of the solution to the Activation Mix and discard \(String(format: "%.0f", discardedVolume)) mL of the remaining solution.")
                        .foregroundStyle(CMTheme.textTertiary)
                }
                .font(.system(size: 10, design: .monospaced))
                .padding(.horizontal, 20)
