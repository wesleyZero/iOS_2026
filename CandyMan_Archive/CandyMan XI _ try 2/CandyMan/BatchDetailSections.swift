    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                hpCumulativeRow("+ Active Water", cumulative: batch.hpAdditionalActivationWater, individual: batch.hpIndividualActiveWater)
                hpCumulativeRow("+ Potassium Sorbate Solution", cumulative: batch.hpKSorbate, individual: batch.hpIndividualKSorbate)
                hpCumulativeRow("+ Citric Acid Solution", cumulative: batch.hpCitricAcid, individual: batch.hpIndividualCitricAcid)
    ...
    private func hpMeasurementsJSON() -> [String: Any] {
        ...
        return [
            ...
            "activationMixture": [
                ...
                "individualActiveWater": opt(batch.hpIndividualActiveWater),
...
struct BatchExperimentalData2Section: View {
    ...
    private var gelatinOverage: Double {
        1.0 + batch.gelatinMixtureOveragePercent / 100.0
    }
    ...
    private var theoCitricAcidMass: Double {
        sortedComponents.first { $0.label == "Citric Acid Solution" }?.massGrams ?? 0
    }
    private var theoActivationWaterMass: Double {
        sortedComponents.first { $0.label == "Active Water" }?.massGrams ?? 0
    }
    private var theoKSorbateMass: Double {
        sortedComponents.first { $0.label == "Potassium Sorbate Solution" }?.massGrams ?? 0
    }
    ...
    private var expActiveWaterMass: Double? { batch.hpIndividualActiveWater }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if isExpanded {
                ...
                if batch.gelatinMixtureOveragePercent > 0 {
                    Text(String(format: "Gelatin theoretical values include %.0f%% overage.", batch.gelatinMixtureOveragePercent))
                        .cmMono10()
                        .foregroundStyle(CMTheme.textTertiary)
                ...
                comparisonRow("Active Water", theoretical: theoActivationWaterMass, experimental: expActiveWaterMass)
                comparisonRow("Potassium Sorbate Solution", theoretical: theoKSorbateMass, experimental: expKSorbateMass)
                comparisonRow("Citric Acid Solution", theoretical: theoCitricAcidMass, experimental: expCitricAcidMass)
    ...
    private func expData2JSON() -> [String: Any] {
        ...
        return [
            ...
            "activationMixture": [
                ...
                "theoActiveWater": theoActivationWaterMass, "expActiveWater": opt(expActiveWaterMass),
...
struct BatchSigFigSection: View {
    ...
    private var sfExpActiveWater: SigFigInfo? {
        guard let water = batch.hpAdditionalActivationWater, let active = batch.hpActive else { return nil }
    ...
    private var sfExpActivationMixTotal: SigFigInfo? {
        ...
        let v2: Double = batch.hpIndividualActiveWater ?? 0
        let v3: Double = batch.hpIndividualKSorbate ?? 0
        let v4: Double = batch.hpIndividualCitricAcid ?? 0
        let v5: Double = batch.hpIndividualFlavorOils ?? 0
        let v6: Double = batch.hpIndividualTerps ?? 0
        let v7: Double = batch.hpIndividualColor ?? 0
...
struct BatchDryWeightSection: View {
    ...
    private var formulationWaterMass: Double? {
        let waterComponents = sortedComponents.filter {
            $0.label == "Water" || $0.label == "Active Water"
        }
