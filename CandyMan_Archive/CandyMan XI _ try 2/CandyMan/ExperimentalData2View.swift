...
struct ExperimentalData2View: View {
    ...
    // Gelatin Mix theoretical sub-components (with overage applied)
    private var theoGelatinMass: Double { result.gelatinMix.components[0].massGrams * gelatinOverage }
    private var theoGelatinWaterMass: Double { result.gelatinMix.components[1].massGrams * gelatinOverage }
    private var theoGelatinMixTotal: Double { result.gelatinMix.totalMassGrams * gelatinOverage }
    ...
    private var theoCitricAcidMass: Double {
        result.activationMix.components.first { $0.label == "Citric Acid Solution" }?.massGrams ?? 0
    }
    private var theoActivationWaterMass: Double {
        result.activationMix.components.first { $0.label == "Active Water" }?.massGrams ?? 0
    }
    private var theoKSorbateMass: Double {
        result.activationMix.components.first { $0.label == "Potassium Sorbate Solution" }?.massGrams ?? 0
    }
    ...
    private var expActiveWaterMass: Double? {
        viewModel.hpIndividualActiveWater
    }
    ...
    private var expActivationMixTotal: Double? {
        guard let a = expActiveMass, let w = expActiveWaterMass,
              let k = expKSorbateMass, let c = expCitricAcidMass,
              let fo = expFlavorOilsMass, let t = expTerpsMass, let co = expColorMass else { return nil }
        return a + w + k + c + fo + t + co
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if isExpanded && viewModel.batchActivated {
                ...
                comparisonRow("Active Water",
                              theoretical: theoActivationWaterMass,
                              experimental: expActiveWaterMass)
                comparisonRow("Potassium Sorbate Solution",
                              theoretical: theoKSorbateMass,
                              experimental: expKSorbateMass)
                comparisonRow("Citric Acid Solution",
