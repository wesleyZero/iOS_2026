...
struct SettingsView: View {
    ...
    private var ipadLeftBatchParams: AnyView {
        ...
        return AnyView(Group {
            ...
            solutionConcentrationsSection(systemConfig: systemConfig).cardStyle()
    ...
    private func solutionConcentrationsSection(systemConfig: SystemConfig) -> some View {
        @Bindable var systemConfig = systemConfig
        return VStack(spacing: 0) {
            Text("Solution Concentrations").cmSectionTitle(accent: systemConfig.designTitle)
                .padding(.horizontal, 16).padding(.vertical, 12)

            HStack {
                Text("Citric acid solution").font(.body)
                if systemConfig.citricAcidSolutionConcentration != 0.50 {
                    Button {
                        CMHaptic.light()
                        withAnimation(.cmSpring) { systemConfig.citricAcidSolutionConcentration = 0.50 }
                    } label: {
                        Image(systemName: "arrow.counterclockwise.circle")
                            .cmResetIcon(color: systemConfig.designAlert)
                    }
                ...
                Spacer()
                NumericField(value: Binding(
                    get: { systemConfig.citricAcidSolutionConcentration * 100.0 },
                    set: { systemConfig.citricAcidSolutionConcentration = $0 / 100.0 }
                ), decimals: 1)
                    .multilineTextAlignment(.trailing).frame(width: 60)
                Text("%").font(.subheadline).foregroundStyle(CMTheme.textSecondary)
            }.cmSettingsRowPadding()
            Divider().padding(.leading, 16)
            HStack {
                Text("K sorbate solution").font(.body)
                if systemConfig.potassiumSorbateSolutionConcentration != 0.50 {
                    Button {
                        CMHaptic.light()
                        withAnimation(.cmSpring) { systemConfig.potassiumSorbateSolutionConcentration = 0.50 }
                    } label: {
                        Image(systemName: "arrow.counterclockwise.circle")
                            .cmResetIcon(color: systemConfig.designAlert)
                    }
                ...
                Spacer()
                NumericField(value: Binding(
                    get: { systemConfig.potassiumSorbateSolutionConcentration * 100.0 },
                    set: { systemConfig.potassiumSorbateSolutionConcentration = $0 / 100.0 }
                ), decimals: 1)
                    .multilineTextAlignment(.trailing).frame(width: 60)
                Text("%").font(.subheadline).foregroundStyle(CMTheme.textSecondary)
            }.cmSettingsRowPadding()
            Text("Mass % of solute in the premade solution (50% = equal parts by mass).")
                .cmFootnote().cmSettingsRowPadding()
        }
    }
