...
struct BatchDetailView: View {
            ...
            if isRegular {
                HStack(alignment: .top, spacing: 0) {
                    LazyVStack(spacing: 12) {
                        ...
                        BatchExperimentalData2Section(batch: batch, copiedConfirmation: $copiedConfirmation, copiedLabel: $copiedLabel).cardStyle()
                        BatchSigFigSection(batch: batch, copiedConfirmation: $copiedConfirmation, copiedLabel: $copiedLabel).cardStyle()
                    }
                    .frame(maxWidth: .infinity, alignment: .top)
                    .fixedSize(horizontal: false, vertical: true)

                    LazyVStack(spacing: 12) {
                        BatchHPMeasurementsSection(batch: batch, copiedConfirmation: $copiedConfirmation, copiedLabel: $copiedLabel).cardStyle()
    ...
    private func buildJSONString() -> String {
        ...
        let waterComponents = sortedComponents.filter { $0.label == "Water" || $0.label == "Active Water" }
