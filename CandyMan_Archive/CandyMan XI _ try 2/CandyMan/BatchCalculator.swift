...
enum BatchCalculator {
    ...
    static func calculate(
        ...
        // 2a  Citric acid solution
        //     Need mCitric grams of citric acid → mSolution = mSolute / concentration.
        //     The solution volume includes both the solid and its dissolved water;
        //     this water displaces sugar-mix volume via vActivation.
        let citricConc = systemConfig.citricAcidSolutionConcentration
        let vCitric = (systemConfig.citricAcidPercent / 100.0) * vMix
        let mCitric = vCitric * systemConfig.densityCitricAcid
        let mCitricSolution = mCitric / citricConc
        let mWaterFromCitric = mCitricSolution - mCitric              // water mass in this solution
        let vWaterFromCitric = mWaterFromCitric / systemConfig.densityWater // water volume in this solution
        let vCitricSolution  = vCitric + vWaterFromCitric              // total solution volume (solid + water)
        activationComponents.append(BatchComponent(
            label: "Citric Acid Solution", massGrams: mCitricSolution, volumeML: vCitricSolution, displayUnit: "g",
            activationCategory: .preservative
        ))

        // 2b  Potassium sorbate solution (same pattern)
        let sorbateConc = systemConfig.potassiumSorbateSolutionConcentration
        let vSorbate = (systemConfig.potassiumSorbatePercent / 100.0) * vMix
        let mSorbate = vSorbate * systemConfig.densityPotassiumSorbate
        let mSorbateSolution = mSorbate / sorbateConc
        let mWaterFromSorbate = mSorbateSolution - mSorbate
        let vWaterFromSorbate = mWaterFromSorbate / systemConfig.densityWater
        let vSorbateSolution  = vSorbate + vWaterFromSorbate
        activationComponents.append(BatchComponent(
            label: "Potassium Sorbate Solution", massGrams: mSorbateSolution, volumeML: vSorbateSolution, displayUnit: "g",
            activationCategory: .preservative
        ))
        ...
        // 2f  Active water — derived from the difference between the
        //     cumulative "Active Water" and "Active" scale readings.
        //     Before measurements are taken this defaults to 0.
        let measuredWaterMass = viewModel.hpIndividualActiveWater ?? 0
        let mActiveWater = max(measuredWaterMass, 0)
        let vActiveWater = mActiveWater / systemConfig.densityWater
        activationComponents.append(BatchComponent(
            label: "Active Water", massGrams: mActiveWater, volumeML: vActiveWater, displayUnit: "ml",
            activationCategory: .preservative
        ))
