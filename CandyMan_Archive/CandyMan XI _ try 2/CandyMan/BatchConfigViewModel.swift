...
final class BatchConfigViewModel {
    ...
    var useMultipleActivations: Bool = false
    /// Currently selected tray index (0-based) in the multi-active picker.
    var selectedTrayIndex: Int = 0

    // MARK: - Per-Tray Configuration (Multi-Active Batch)

    /// Snapshot of all per-tray fields. When multi-active is on, each tray gets its own config.
    struct TrayConfig {
        // Recipe inputs
        ...
        // Post-calculate: activation mix HP readings
        ...
        // Post-calculate: activation density calibration
        ...
        // Batch activation state
        var batchActivated: Bool = false
    }

    /// Per-tray configs; only used when `useMultipleActivations` is true.
    var perTrayConfigs: [TrayConfig] = []

    /// Cached per-tray BatchResults, populated at calculate-time when multi-active is on.
    var perTrayResults: [BatchResult] = []

    /// True when every tray in perTrayConfigs has been activated.
    var allTraysActivated: Bool {
        guard useMultipleActivations, !perTrayConfigs.isEmpty else { return false }
        return perTrayConfigs.allSatisfy { $0.batchActivated }
    }

    /// Saves the view model's current per-tray fields into `perTrayConfigs[index]`.
    func saveCurrentTrayConfig(at index: Int) {
        guard useMultipleActivations, index < perTrayConfigs.count else { return }
        var c = TrayConfig()
        c.selectedActive = selectedActive
        c.activeConcentration = activeConcentration
        c.units = units
        c.lsdUgPerTab = lsdUgPerTab
        c.additionalActiveWaterML = additionalActiveWaterML
        c.gelatinPercentage = gelatinPercentage
        c.overageFactor = overageFactor
        c.selectedColors = selectedColors
        c.colorsLocked = colorsLocked
        c.colorVolumePercent = colorVolumePercent
        c.colorCompositionLocked = colorCompositionLocked
        c.selectedFlavors = selectedFlavors
        c.oilsLocked = oilsLocked
        c.terpenesLocked = terpenesLocked
        c.flavorSourceTab = flavorSourceTab
        c.waterRatioGelatinToSugar = waterRatioGelatinToSugar
        c.terpeneVolumePPM = terpeneVolumePPM
        c.flavorOilVolumePercent = flavorOilVolumePercent
        c.flavorCompositionLocked = flavorCompositionLocked
        c.hpActive = hpActive
        c.hpCitricAcid = hpCitricAcid
        c.hpKSorbate = hpKSorbate
        c.hpActivationWater = hpActivationWater
        c.hpAdditionalActivationWater = hpAdditionalActivationWater
        c.hpFlavorOils = hpFlavorOils
        c.hpColor = hpColor
        c.hpTerps = hpTerps
        c.hpActivationTrayResidue = hpActivationTrayResidue
        c.hpActivationTrayID = hpActivationTrayID
        c.hpActivationScaleID = hpActivationScaleID
        c.hpActivationStirBarID = hpActivationStirBarID
        c.hpMoldsTrayID = hpMoldsTrayID
        c.hpMoldsScaleID = hpMoldsScaleID
        c.weightTrayClean = weightTrayClean
        c.weightTrayPlusResidue = weightTrayPlusResidue
        c.weightMoldsFilled = weightMoldsFilled
        c.extraGummyMixGrams = extraGummyMixGrams
        c.densitySyringeCleanActive = densitySyringeCleanActive
        c.densitySyringePlusActiveMass = densitySyringePlusActiveMass
        c.densitySyringePlusActiveVol = densitySyringePlusActiveVol
        c.batchActivated = batchActivated
        perTrayConfigs[index] = c
    }

    /// Loads per-tray fields from `perTrayConfigs[index]` into the view model.
    func loadTrayConfig(at index: Int) {
        guard useMultipleActivations, index < perTrayConfigs.count else { return }
        let c = perTrayConfigs[index]
        selectedActive = c.selectedActive
        activeConcentration = c.activeConcentration
        units = c.units
        lsdUgPerTab = c.lsdUgPerTab
        additionalActiveWaterML = c.additionalActiveWaterML
        gelatinPercentage = c.gelatinPercentage
        overageFactor = c.overageFactor
        selectedColors = c.selectedColors
        colorsLocked = c.colorsLocked
        colorVolumePercent = c.colorVolumePercent
        colorCompositionLocked = c.colorCompositionLocked
        selectedFlavors = c.selectedFlavors
        oilsLocked = c.oilsLocked
        terpenesLocked = c.terpenesLocked
        flavorSourceTab = c.flavorSourceTab
        waterRatioGelatinToSugar = c.waterRatioGelatinToSugar
        terpeneVolumePPM = c.terpeneVolumePPM
        flavorOilVolumePercent = c.flavorOilVolumePercent
        flavorCompositionLocked = c.flavorCompositionLocked
        hpActive = c.hpActive
        hpCitricAcid = c.hpCitricAcid
        hpKSorbate = c.hpKSorbate
        hpActivationWater = c.hpActivationWater
        hpAdditionalActivationWater = c.hpAdditionalActivationWater
        hpFlavorOils = c.hpFlavorOils
        hpColor = c.hpColor
        hpTerps = c.hpTerps
        hpActivationTrayResidue = c.hpActivationTrayResidue
        hpActivationTrayID = c.hpActivationTrayID
        hpActivationScaleID = c.hpActivationScaleID
        hpActivationStirBarID = c.hpActivationStirBarID
        hpMoldsTrayID = c.hpMoldsTrayID
        hpMoldsScaleID = c.hpMoldsScaleID
        weightTrayClean = c.weightTrayClean
        weightTrayPlusResidue = c.weightTrayPlusResidue
        weightMoldsFilled = c.weightMoldsFilled
        extraGummyMixGrams = c.extraGummyMixGrams
        densitySyringeCleanActive = c.densitySyringeCleanActive
        densitySyringePlusActiveMass = c.densitySyringePlusActiveMass
        densitySyringePlusActiveVol = c.densitySyringePlusActiveVol
        batchActivated = c.batchActivated
    }

    /// Switches from one tray to another, saving the old and loading the new.
    func switchTray(from oldIndex: Int, to newIndex: Int) {
        guard useMultipleActivations else { return }
        ensureTrayConfigs()
        saveCurrentTrayConfig(at: oldIndex)
        loadTrayConfig(at: newIndex)
    }

    /// Ensures `perTrayConfigs` has entries for all trays, initializing from current values.
    func ensureTrayConfigs() {
        while perTrayConfigs.count < trayCount {
            var c = TrayConfig()
            c.selectedActive = selectedActive
            c.activeConcentration = activeConcentration
            c.units = units
            ...
            c.flavorOilVolumePercent = flavorOilVolumePercent
            c.flavorCompositionLocked = flavorCompositionLocked
            perTrayConfigs.append(c)
        }
        // Trim if trays were reduced
        if perTrayConfigs.count > trayCount {
            perTrayConfigs = Array(perTrayConfigs.prefix(trayCount))
        }
    }
    ...
    /// When multi-active is on, returns count for a single tray (per-tray calculation).
    func totalGummies(using systemConfig: SystemConfig) -> Int {
        let spec = systemConfig.spec(for: selectedShape)
        if useMultipleActivations {
            return spec.count
        }
    ...
    private func clearMeasurements() {
        ...
        hpAdditionalActivationWater = nil
        hpFlavorOils                = nil
        hpColor                     = nil
        hpTerps                     = nil
    ...
    func prePopulateRecommendedScales(systemConfig: SystemConfig) {
        ...
        let gelatinMass = result.gelatinMix.totalMassGrams * gelatinOverage
        let gelatinVol  = result.gelatinMix.totalVolumeML * gelatinOverage
    ...
    /// Tare mass for the substrate beaker + stir bar from the selected equipment in SystemConfig.
    /// Falls back to recommended beaker / first stir bar when no explicit selection exists,
    /// matching the defaults shown in WeightMeasurementsView.
    func hpSubstrateTare(systemConfig: SystemConfig) -> Double {
        let result = BatchCalculator.calculate(viewModel: self, systemConfig: systemConfig)
        let sugarOverage = 1.0 + systemConfig.sugarMixtureOveragePercent / 100.0
        let gelatinOverage = 1.0 + systemConfig.gelatinMixtureOveragePercent / 100.0
        let substrateVol = result.gelatinMix.totalVolumeML * gelatinOverage + result.sugarMix.totalVolumeML * sugarOverage

        let containerID = hpSubstrateBeakerID
            ?? systemConfig.recommendedBeaker(forVolumeML: substrateVol)?.id
        let beaker = containerID.map { systemConfig.containerTare(for: $0) } ?? 0

        let stirBarID = hpSubstrateStirBarID ?? systemConfig.stirBars.first?.id
        let stirBar = stirBarID.map { systemConfig.stirBarMass(for: $0) } ?? 0

        return beaker + stirBar
    }

    /// Tare mass for the sugar mix beaker + stir bar from the selected equipment in SystemConfig.
    /// Falls back to recommended beaker / first stir bar when no explicit selection exists,
    /// matching the defaults shown in WeightMeasurementsView.
    func hpSugarMixTare(systemConfig: SystemConfig) -> Double {
        ...
        let containerID = hpSugarMixBeakerID
            ?? systemConfig.recommendedBeaker(forVolumeML: sugarVol)?.id
        let beaker = containerID.map { systemConfig.containerTare(for: $0) } ?? 0

        let stirBarID = hpSugarMixStirBarID ?? systemConfig.stirBars.first?.id
        let stirBar = stirBarID.map { systemConfig.stirBarMass(for: $0) } ?? 0
    ...
    /// Falls back to recommended beaker when no explicit selection exists,
    /// matching the defaults shown in WeightMeasurementsView.
    func hpActivationTare(systemConfig: SystemConfig) -> Double {
        let result = BatchCalculator.calculate(viewModel: self, systemConfig: systemConfig)
        let activVol = result.activationMix.totalVolumeML

        let containerID = hpActivationTrayID
            ?? systemConfig.recommendedBeaker(forVolumeML: activVol)?.id
        let beaker = containerID.map { systemConfig.containerTare(for: $0) } ?? 0
        let stirBarID = hpActivationStirBarID ?? systemConfig.stirBars.first?.id
        let stirBar = stirBarID.map { systemConfig.stirBarMass(for: $0) } ?? 0
        return beaker + stirBar
    }
    ...
    /// HP Gelatin Mixture net mass = last reading (gelatin) − container tare
    func hpGelatinMixtureTotal(systemConfig: SystemConfig) -> Double? {
        guard let last = hpGelatin else { return nil }
        let tare = hpSubstrateTare(systemConfig: systemConfig)
        return last - tare
    }

    /// HP Sugar Mixture net mass = last reading (granulated) − container tare
    func hpSugarMixtureTotal(systemConfig: SystemConfig) -> Double? {
        guard let last = hpGranulated else { return nil }
    ...
    func hpActivationMixtureTotal(systemConfig: SystemConfig) -> Double? {
        guard let last = hpColor else { return nil }
    ...
    // Gelatin order: Water → Gelatin
    func hpIndividualGelatinWater(systemConfig: SystemConfig) -> Double? {
        guard let water = hpGelatinWater else { return nil }
        let tare = hpSubstrateTare(systemConfig: systemConfig)
        return water - tare
    }
    ...
    // Sugar order: Water → Glucose Syrup → Granulated
    func hpIndividualSugarWater(systemConfig: SystemConfig) -> Double? {
    ...
    func hpIndividualActive(systemConfig: SystemConfig) -> Double? {
        guard let active = hpActive else { return nil }
        let tare = hpActivationTare(systemConfig: systemConfig)
        return active - tare
    }

    // Order: Active → Active Water → K Sorbate Solution → Citric Acid Solution → Flavor Oils → Terps → Color
    var hpIndividualActiveWater: Double? {
        guard let water = hpAdditionalActivationWater, let active = hpActive else { return nil }
        return water - active
    }

    var hpIndividualKSorbate: Double? {
        guard let k = hpKSorbate, let water = hpAdditionalActivationWater else { return nil }
        return k - water
    }

    var hpIndividualCitricAcid: Double? {
        guard let citric = hpCitricAcid, let k = hpKSorbate else { return nil }
        return citric - k
    }

    var hpIndividualFlavorOils: Double? {
        guard let oils = hpFlavorOils, let citric = hpCitricAcid else { return nil }
        return oils - citric
    }

    var hpIndividualTerps: Double? {
        guard let terps = hpTerps, let oils = hpFlavorOils else { return nil }
        return terps - oils
    }

    var hpIndividualColor: Double? {
        guard let color = hpColor, let terps = hpTerps else { return nil }
        return color - terps
    }
