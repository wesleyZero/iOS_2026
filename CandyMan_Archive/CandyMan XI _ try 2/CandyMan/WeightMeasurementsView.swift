            ...
            VStack(spacing: 0) {
                ...
                // MARK: Activation Mixture
                    ...
                    hpSectionBox {
                        ...
                        let mCitricSolution = activComps.first(where: { $0.label == "Citric Acid Solution" })?.massGrams ?? 0
                        let mKSorbateSolution = activComps.first(where: { $0.label == "Potassium Sorbate Solution" })?.massGrams ?? 0
                        let mActiveWater = activComps.first(where: { $0.label == "Active Water" })?.massGrams ?? 0
                        ...
                        let expectedActiveWater: Double? = viewModel.hpActive.map { $0 + mActiveWater }
                        let expectedKSorbate: Double? = viewModel.hpAdditionalActivationWater.map { $0 + mKSorbateSolution }
                        let expectedCitric: Double? = viewModel.hpKSorbate.map { $0 + mCitricSolution }
                        let expectedFlavorOils: Double? = viewModel.hpCitricAcid.map { $0 + mFlavorOils }
                        ...
                        subWeightRow("+ Active Water", value: $viewModel.hpAdditionalActivationWater, resolution: activRes,
                                     expected: viewModel.hpActive != nil && viewModel.hpAdditionalActivationWater == nil ? expectedActiveWater : nil,
                                     individualMass: mActiveWater)
                        subWeightRow("+ Potassium Sorbate Solution", value: $viewModel.hpKSorbate, resolution: activRes,
                                     expected: viewModel.hpAdditionalActivationWater != nil && viewModel.hpKSorbate == nil ? expectedKSorbate : nil,
                                     individualMass: mKSorbateSolution)
                        subWeightRow("+ Citric Acid Solution", value: $viewModel.hpCitricAcid, resolution: activRes,
                                     expected: viewModel.hpKSorbate != nil && viewModel.hpCitricAcid == nil ? expectedCitric : nil,
                                     individualMass: mCitricSolution)
                        subWeightRow("+ Flavor Oils", value: $viewModel.hpFlavorOils, resolution: activRes,
                                     expected: viewModel.hpCitricAcid != nil && viewModel.hpFlavorOils == nil ? expectedFlavorOils : nil,
                        ...
                        let activNetTotal: Double? = {
                            guard let last = viewModel.hpColor else { return nil }
