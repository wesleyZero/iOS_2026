...
struct BatchOutputView: View {
    ...
    private func mixSection(_ mix: MixGroup) -> some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                Spacer()
                if batchActivated {
                    Text(String(format: "%.3f", mix.totalMassGrams)).cmValueSlot()
                } else {
                    Text("██████").cmValueSlot(color: CMTheme.textTertiary.opacity(0.4))
                }
                Text("g").cmUnitSlot()
            }
            .cmDataRowPadding()
    ...
    private func overageMixSection(_ mix: MixGroup, overagePercent: Double) -> some View {
        ...
        return VStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                if batchActivated {
                    if overagePercent > 0 {
                        Text(String(format: "%.3f", mix.totalMassGrams * factor))
                            .cmValueSlot(width: 60, color: fuchsiaFlare)
                    }
                    Text(String(format: "%.3f", mix.totalMassGrams)).cmValueSlot()
                } else {
    ...
    private func componentRow(_ comp: BatchComponent) -> some View {
        ...
        let isActivationWaterRow = comp.label == "Active Water"
        ...
        return VStack(spacing: 0) {
            ...
            // LSD kept transfer water annotation
            if isActivationWaterRow && isLSD && keptVolume > 0 {
                VStack(spacing: 1) {
                    HStack(spacing: 6) {
    ...
    private var activateBatchButton: some View {
        Button {
            ...
            withAnimation(.cmSpring) {
                ...
                // Sync activation state back to perTrayConfigs for allTraysActivated check
                if viewModel.useMultipleActivations {
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 0) {
                    ...
                    fullDivider
                    if viewModel.useMultipleActivations {
                        fullMixSection(result.sugarMix)
                    } else {
    ...
    private func fullMixSection(_ mix: MixGroup) -> some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 10) {
                ...
                Spacer()
                Text(String(format: "%.3f", mix.totalMassGrams))
