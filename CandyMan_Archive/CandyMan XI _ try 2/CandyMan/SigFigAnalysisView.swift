...
struct SigFigAnalysisView: View {
    ...
    private var sfExpActiveWater: SigFigInfo? {
        guard let water = viewModel.hpAdditionalActivationWater, let active = viewModel.hpActive else { return nil }
        return SigFigs.sfOfDifference(water, resA: activationRes, minus: active, resB: activationRes)
    }

    private var sfExpKSorbate: SigFigInfo? {
        guard let k = viewModel.hpKSorbate, let water = viewModel.hpAdditionalActivationWater else { return nil }
        return SigFigs.sfOfDifference(k, resA: activationRes, minus: water, resB: activationRes)
    }

    private var sfExpCitricAcid: SigFigInfo? {
        guard let citric = viewModel.hpCitricAcid, let k = viewModel.hpKSorbate else { return nil }
        return SigFigs.sfOfDifference(citric, resA: activationRes, minus: k, resB: activationRes)
    }

    private var sfExpFlavorOils: SigFigInfo? {
        guard let oils = viewModel.hpFlavorOils, let citric = viewModel.hpCitricAcid else { return nil }
        return SigFigs.sfOfDifference(oils, resA: activationRes, minus: citric, resB: activationRes)
    }

    private var sfExpTerps: SigFigInfo? {
        guard let terps = viewModel.hpTerps, let oils = viewModel.hpFlavorOils else { return nil }
        return SigFigs.sfOfDifference(terps, resA: activationRes, minus: oils, resB: activationRes)
    }

    private var sfExpColor: SigFigInfo? {
        guard let color = viewModel.hpColor, let terps = viewModel.hpTerps else { return nil }
        return SigFigs.sfOfDifference(color, resA: activationRes, minus: terps, resB: activationRes)
    }

    private var sfExpActivationMixTotal: SigFigInfo? {
        guard let a = sfExpActive, let w = sfExpActiveWater, let k = sfExpKSorbate,
              let c = sfExpCitricAcid, let fo = sfExpFlavorOils,
              let t = sfExpTerps, let co = sfExpColor else { return nil }
        let resultDP = SigFigs.addSubtract(a, w, k, c, fo, t, co)
        let v1: Double = viewModel.hpIndividualActive(systemConfig: systemConfig) ?? 0
        let v2: Double = viewModel.hpIndividualActiveWater ?? 0
        let v3: Double = viewModel.hpIndividualKSorbate ?? 0
        let v4: Double = viewModel.hpIndividualCitricAcid ?? 0
        let v5: Double = viewModel.hpIndividualFlavorOils ?? 0
        let v6: Double = viewModel.hpIndividualTerps ?? 0
        let v7: Double = viewModel.hpIndividualColor ?? 0
        let value = v1 + v2 + v3 + v4 + v5 + v6 + v7
        let str = SigFigs.formatDP(value, decimalPlaces: resultDP)
        return SigFigs.count(from: str)
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if isExpanded && viewModel.batchActivated {
                ...
                sfRow("Active Water", info: sfExpActiveWater, unit: "g",
                      detail: consecutiveSubDetail(label: "Active Water Mass", a: viewModel.hpAdditionalActivationWater, aLabel: "Reading after active water", b: viewModel.hpActive, bLabel: "Reading after active", res: activationRes, result: sfExpActiveWater))
                sfRow("Potassium Sorbate Solution", info: sfExpKSorbate, unit: "g",
                      detail: consecutiveSubDetail(label: "K Sorbate Solution Mass", a: viewModel.hpKSorbate, aLabel: "Reading after K sorbate solution", b: viewModel.hpAdditionalActivationWater, bLabel: "Reading after active water", res: activationRes, result: sfExpKSorbate))
                sfRow("Citric Acid Solution", info: sfExpCitricAcid, unit: "g",
                      detail: consecutiveSubDetail(label: "Citric Acid Solution Mass", a: viewModel.hpCitricAcid, aLabel: "Reading after citric acid solution", b: viewModel.hpKSorbate, bLabel: "Reading after K sorbate solution", res: activationRes, result: sfExpCitricAcid))
                sfRow("Flavor Oils", info: sfExpFlavorOils, unit: "g",
                      detail: consecutiveSubDetail(label: "Flavor Oils Mass", a: viewModel.hpFlavorOils, aLabel: "Reading after flavor oils", b: viewModel.hpCitricAcid, bLabel: "Reading after citric acid solution", res: activationRes, result: sfExpFlavorOils))
                sfRow("Terps", info: sfExpTerps, unit: "g",
                      detail: consecutiveSubDetail(label: "Terps Mass", a: viewModel.hpTerps, aLabel: "Reading after terps", b: viewModel.hpFlavorOils, bLabel: "Reading after flavor oils", res: activationRes, result: sfExpTerps))
                sfRow("Color", info: sfExpColor, unit: "g",
                      detail: consecutiveSubDetail(label: "Color Mass", a: viewModel.hpColor, aLabel: "Reading after color", b: viewModel.hpTerps, bLabel: "Reading after terps", res: activationRes, result: sfExpColor))
                ThemedDivider(indent: 20).padding(.vertical, 4)
                sfRow("Activation Mix Total", info: sfExpActivationMixTotal, unit: "g", bold: true,
                      detail: additionDetail(label: "Activation Mix Total", components: [("Active", sfExpActive), ("Active Water", sfExpActiveWater), ("K Sorbate Soln", sfExpKSorbate), ("Citric Acid Soln", sfExpCitricAcid), ("Flavor Oils", sfExpFlavorOils), ("Terps", sfExpTerps), ("Color", sfExpColor)], totalValue: sfExpActivationMixTotal?.value, totalSF: sfExpActivationMixTotal))
                    .background(CMTheme.totalRowBG)
