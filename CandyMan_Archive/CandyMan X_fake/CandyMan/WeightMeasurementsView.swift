    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                Button {
                    ...
                    Image(systemName: viewModel.highPrecisionMode ? "scope" : "scope")
                        .font(.system(size: 14, weight: .medium))
                        .foregroundStyle(viewModel.highPrecisionMode ? Color(red: 0.0, green: 0.85, blue: 1.0) : CMTheme.textTertiary)
            ...
            VStack(spacing: 0) {
                ...
                // High-precision: Gelatin Mix sub-fields
                if viewModel.highPrecisionMode {
                    subWeightRow("+ Gelatin", value: $viewModel.hpGelatin, resolution: .thousandthGram)
                    subWeightRow("+ Water", value: $viewModel.hpGelatinWater, resolution: .thousandthGram)
                }

                weightRow("Beaker + Gelatin Mix", value: $viewModel.weightBeakerPlusGelatin, resolution: systemConfig.resolutionBeakerPlusGelatin)

                // High-precision: Sugar Mix sub-fields
                if viewModel.highPrecisionMode {
                    subWeightRow("Beaker B (Empty)", value: $viewModel.hpBeakerBEmpty, resolution: .thousandthGram)
                    subWeightRow("+ Granulated", value: $viewModel.hpGranulated, resolution: .thousandthGram)
                    subWeightRow("+ Water", value: $viewModel.hpSugarWater, resolution: .thousandthGram)
                }

                weightRow("Substrate + Sugar Mix", value: $viewModel.weightBeakerPlusSugar, resolution: systemConfig.resolutionBeakerPlusSugar)

                // High-precision: Activation Mix sub-fields
                if viewModel.highPrecisionMode {
                    subWeightRow("Activation Mixing Tray", value: $viewModel.hpActivationTray, resolution: .thousandthGram)
                    subWeightRow("Potassium Sorbate", value: $viewModel.hpPotassiumSorbate, resolution: .thousandthGram)
                    subWeightRow("Citric Acid", value: $viewModel.hpCitricAcid, resolution: .thousandthGram)
                    subWeightRow("+ Color, Terp, Act. Water", value: $viewModel.hpColorTerpWater, resolution: .thousandthGram)
                }
    ...
    private func subWeightRow(_ label: String, value: Binding<Double?>, resolution: MeasurementResolution) -> some View {
        ...
        return HStack(spacing: 6) {
            Text(label)
                ...
                .foregroundStyle(Color(red: 0.0, green: 0.85, blue: 1.0).opacity(0.8))
                .lineLimit(1)
                .minimumScaleFactor(0.7)
            Spacer()
            OptionalNumericField(value: value, decimals: decimals)
                .multilineTextAlignment(.trailing)
                .font(.system(size: 11, design: .monospaced))
                .foregroundStyle(Color(red: 0.0, green: 0.85, blue: 1.0).opacity(0.6))
                .frame(width: 80)
            Text("g")
                .font(.system(size: 11, design: .monospaced))
                .foregroundStyle(CMTheme.textTertiary)
