...
struct SettingsView: View {
            ...
            Button {
                ...
                HStack {
                    ...
                    Image(systemName: "chevron.down")
                        ...
                        .rotationEffect(.degrees(showTrays ? -180 : 0))
                        .animation(.cmExpand, value: showTrays)
                }
    ...
    private func transferLiquidsSection(systemConfig: SystemConfig) -> some View {
        @Bindable var systemConfig = systemConfig
        return VStack(spacing: 0) {
            HStack { Text("LSD Transfer Liquid").font(.headline).foregroundStyle(systemConfig.accent); Spacer() }
                .padding(.horizontal, 16).padding(.vertical, 12)
            HStack {
    ...
    private func defaultUgPerTabSection(systemConfig: SystemConfig) -> some View {
        @Bindable var systemConfig = systemConfig
        return VStack(spacing: 0) {
            HStack { Text("LSD Tab Strength").font(.headline).foregroundStyle(systemConfig.accent); Spacer() }
                .padding(.horizontal, 16).padding(.vertical, 12)
            HStack {
    ...
    private func overageSection(viewModel: BatchConfigViewModel, systemConfig: SystemConfig) -> some View {
        ...
        return VStack(spacing: 0) {
            ...
            HStack {
                ...
                if systemConfig.gelatinMixtureOveragePercent != 5.0 {
                    Button {
                        CMHaptic.light()
                        withAnimation(.cmSpring) { systemConfig.gelatinMixtureOveragePercent = 5.0 }
                    } label: {
                        Image(systemName: "arrow.counterclockwise.circle")
                            .font(.system(size: 14, weight: .medium))
                            .foregroundStyle(Color(red: 0.929, green: 0.278, blue: 0.290))
        ...
        return VStack(spacing: 0) {
            ...
            Button {
                ...
                HStack {
                    ...
                    Image(systemName: "chevron.down")
                        .font(.subheadline).foregroundStyle(CMTheme.textTertiary)
                        .rotationEffect(.degrees(showDensities ? -180 : 0))
                        .animation(.cmExpand, value: showDensities)
                }
                .contentShape(Rectangle())
                .padding(.horizontal, 16).padding(.vertical, 12)
            }
    ...
    private func measurementResolutionsSection(systemConfig: SystemConfig) -> some View {
        ...
        return VStack(spacing: 0) {
            ...
            Button {
                ...
                HStack {
                    Text("Measurement Resolutions").font(.headline).foregroundStyle(systemConfig.accent)
                    Spacer()
                    Image(systemName: "chevron.down")
                        .font(.subheadline).foregroundStyle(CMTheme.textTertiary)
                        .rotationEffect(.degrees(showResolutions ? -180 : 0))
                        .animation(.cmExpand, value: showResolutions)
                }
                .contentShape(Rectangle())
                .padding(.horizontal, 16).padding(.vertical, 12)
            }
