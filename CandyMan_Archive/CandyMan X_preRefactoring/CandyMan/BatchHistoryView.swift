                ...
                if let message = syncResultMessage {
                    Text(message)
                        ...
                        .background(
                            Capsule()
                                .fill(CMTheme.cardBG)
                                .overlay(Capsule().stroke(CMTheme.cardStroke, lineWidth: 0.5))
                        )
    ...
    private var headerSection: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                .buttonStyle(.plain)
                GlassCopyButton { copyJSON(batchConfigJSON()) }
            }
            .padding(.horizontal, 16).padding(.vertical, 12)
            ...
            if batchInfoExpanded {
                ...
                if batch.activeName == "LSD" && batch.lsdUgPerTab > 0 {
                    ...
                    summaryRow("Transfer Water", value: String(format: "%.3f mL", batch.lsdTransferWater_mL))
                }

                // Additional water
                if batch.additionalActiveWater_mL > 0 {
                    summaryRow("Additional Water", value: String(format: "%.1f mL", batch.additionalActiveWater_mL))
                }
    ...
    private var copyDataSection: some View {
        VStack(spacing: 0) {
            Button {
                ...
                HStack {
                    Text("Copy Data").font(.headline).foregroundStyle(systemConfig.accent)
