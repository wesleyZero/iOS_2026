...
class DehydrationContainer {
    ...
    init(label: String, tareWeight_g: Double = 0) {
        self.label = label
        self.tareWeight_g = tareWeight_g
    }
}
...
class TareWeightRecord {
    var containerLabel: String   // "A" through "Z"
    var weight_g: Double
    var date: Date

    init(containerLabel: String, weight_g: Double, date: Date = .now) {
...
class SavedBatch {
    ...
    var additionalActiveWater_mL: Double = 0

    // LSD-specific parameters
    var lsdUgPerTab: Double = 0
    var lsdTransferWater_mL: Double = 0
    ...
    init(
        ...
        self.extraGummyMix_g = extraGummyMix_g
...
struct SavedBatchDTO: Codable {
    ...
    var lsdTransferWater_mL: Double
    var weightBeakerEmpty: Double?
    var weightBeakerPlusGelatin: Double?
    var weightBeakerPlusSugar: Double?
    var weightBeakerPlusActive: Double?
    ...
    var wetGummyMass_g: Double?
    var dehydrationTareWeight_g: Double
    var densitySyringeCleanSugar: Double?
    var densitySyringePlusSugarMass: Double?
    var densitySyringePlusSugarVol: Double?
    var densitySyringeCleanGelatin: Double?
...
extension SavedBatch {
    ...
    static func from(dto: SavedBatchDTO, context: ModelContext) {
        let batch = SavedBatch(
            ...
            extraGummyMix_g: dto.extraGummyMix_g,
        ...
        batch.additionalActiveWater_mL = dto.additionalActiveWater_mL
        batch.lsdUgPerTab = dto.lsdUgPerTab
        batch.lsdTransferWater_mL = dto.lsdTransferWater_mL
        ...
        if let containers = dto.dehydrationContainers, !containers.isEmpty {
            batch.dehydrationContainers = containers.map { cDTO in
                let container = DehydrationContainer(label: cDTO.label, tareWeight_g: cDTO.tareWeight_g)
            ...
            let container = DehydrationContainer(label: "A", tareWeight_g: dto.dehydrationTareWeight_g)
...
extension BatchConfigViewModel {
    func makeSavedBatch(name: String, batchID: String, result: BatchResult, systemConfig: SystemConfig) -> SavedBatch {
        ...
        batch.additionalActiveWater_mL = additionalActiveWater_mL
        batch.lsdUgPerTab = lsdUgPerTab
        batch.lsdTransferWater_mL = systemConfig.lsdTransferWater_mL
