                ...
                ForEach(categoryItems.indices, id: \.self) { i in
                    HStack {
                        Text(categoryItems[i].label)
                            ...
                            .lineLimit(1).minimumScaleFactor(0.8)
                        Spacer()
                        Text(String(format: "%.3f", categoryItems[i].mass_g))
                            .font(.system(size: 11, design: .monospaced)).foregroundStyle(CMTheme.textSecondary)
                            .frame(width: 70, alignment: .trailing)
                        Text(String(format: "%.3f", categoryItems[i].volume_mL))
                            .font(.system(size: 11, design: .monospaced)).foregroundStyle(CMTheme.textSecondary)
                            .frame(width: 70, alignment: .trailing)
                    }
                    .padding(.horizontal, 20).padding(.vertical, 2)
                ...
                ForEach(categoryItems.indices, id: \.self) { i in
                    HStack {
                        Text(categoryItems[i].label)
                            ...
                            .lineLimit(1).minimumScaleFactor(0.8)
                        Spacer()
                        Text(String(format: "%.3f", pct(categoryItems[i].mass_g, of: totalMass)))
                            .font(.system(size: 11, design: .monospaced)).foregroundStyle(CMTheme.textSecondary)
                            .frame(width: 70, alignment: .trailing)
                        Text(String(format: "%.3f", pct(categoryItems[i].volume_mL, of: totalVol)))
                            .font(.system(size: 11, design: .monospaced)).foregroundStyle(CMTheme.textSecondary)
                            .frame(width: 70, alignment: .trailing)
                    }
                    .padding(.horizontal, 20).padding(.vertical, 2)
...
struct BatchMeasurementsSection: View {
        ...
        VStack(spacing: 0) {
            HStack {
                Button {
                    ...
                    HStack {
                        Text("Measurements").font(.headline).foregroundStyle(systemConfig.accent)
                        Image(systemName: isExpanded ? "chevron.up" : "chevron.down")
                            .font(.system(size: 12, weight: .semibold))
                            .foregroundStyle(Color(white: 0.45))
                        Spacer()
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 0) {
                    ...
                    savedWeightRow("Extra Gummy Mix",         value: batch.extraGummyMix_g)
                }
    ...
    private func measurementsJSON() -> [String: Any] {
        ...
        return [
            ...
            "transferToMold": ["syringeClean": opt(batch.weightSyringeEmpty), "syringePlusMix": opt(batch.weightSyringeWithMix), "syringeMixVol_mL": opt(batch.volumeSyringeGummyMix), "syringeResidue": opt(batch.weightSyringeResidue), "beakerResidue": opt(batch.weightBeakerResidue), "trayClean": opt(batch.weightTrayClean), "trayPlusResidue": opt(batch.weightTrayPlusResidue), "moldsFilled": opt(batch.weightMoldsFilled), "extraGummyMix_g": opt(batch.extraGummyMix_g)],
            "densities": [
...
struct BatchCalculationsSection: View {
    ...
    private var massTotalLossWithSurplus: Double? {
        ...
        return base + (batch.extraGummyMix_g ?? 0.0) + (massTrayResidue ?? 0.0)
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                savedCalcRow("Gummy Mixture Surplus",     value: batch.extraGummyMix_g,              unit: "g")
    ...
    private func calculationsJSON() -> [String: Any] {
        ...
        return [
            ...
            "losses": ["beakerResidue": opt(batch.calcMassBeakerResidue), "syringeResidue": opt(batch.calcMassSyringeResidue), "gummyMixtureSurplus": opt(batch.extraGummyMix_g), "trayResidue": opt(massTrayResidue), "totalResidue": opt(massTotalLossWithSurplus ?? batch.calcMassTotalLoss), "lostActive": opt(batch.calcActiveLoss)],
...
struct BatchDryWeightSection: View {
    ...
    private static let containerLabels = (0..<26).map { String(UnicodeScalar($0 + 65)!) } // A–Z

    @State private var isExpanded = true
    @State private var selectedContainerLabel: String = "A"
    ...
    private var currentTareWeight: Double {
        selectedContainer?.tareWeight_g ?? 0
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                Button {
                    ...
                    HStack {
                        Text("Dehydration Tracking").font(.headline).foregroundStyle(systemConfig.accent)
                        Image(systemName: isExpanded ? "chevron.up" : "chevron.down")
                            .font(.system(size: 12, weight: .semibold))
                            .foregroundStyle(Color(white: 0.45))
                        Button {
                            ...
                            Image(systemName: batch.dehydrationLocked ? "lock.fill" : "lock.open.fill")
                                ...
                                .foregroundStyle(batch.dehydrationLocked ? Color(red: 0.929, green: 0.278, blue: 0.290) : CMTheme.textTertiary)
                        }
                        .buttonStyle(.plain)
                        Spacer()
                    }
            ...
            HStack(spacing: 8) {
                ...
                VStack(spacing: 6) {
                    HStack(spacing: 6) {
                        Text("Tare")
                            ...
                            .foregroundStyle(CMTheme.textSecondary)
                        TextField("0.000", text: $tareWeightText)
                    ...
                    HStack(spacing: 8) {
                        ...
                        Button {
                            ...
                            Label("Save", systemImage: "square.and.arrow.down")
                                ...
                                .background(systemConfig.accent.opacity(0.8))
                                .cornerRadius(6)
                        }
                        .buttonStyle(.plain)
                ...
                Picker("Container", selection: $selectedContainerLabel) {
                    ForEach(Self.containerLabels, id: \.self) { letter in
                        let hasData = batch.dehydrationContainers.contains { $0.label == letter && !$0.readings.isEmpty }
                        Text(letter)
            ...
            // Add new reading
            HStack(spacing: 8) {
                TextField("Total mass on scale (g)", text: $newDryMass)
                    .keyboardType(.decimalPad)
                    .font(.system(size: 14, design: .monospaced))
                    .foregroundStyle(CMTheme.textPrimary)
                    .padding(10)
                    ...
                    if let totalMass = Double(newDryMass), totalMass > 0 {
                        ...
                        withAnimation(.cmSpring) {
                            ...
                            container.readings.append(
                                DryWeightReading(mass_g: netMass, timestamp: .now)
                            )
                        }
                        newDryMass = ""
                    }
            ...
            if !currentReadings.isEmpty {
                ...
                ForEach(currentReadings.indices, id: \.self) { i in
                    ...
                    HStack(spacing: 4) {
                        ...
                        Spacer()
                        Text(String(format: "%.1f g", entry.mass_g))
                            .font(.system(size: 10, design: .monospaced))
                            .foregroundStyle(CMTheme.textPrimary)
        ...
        .onChange(of: selectedContainerLabel) { _, newLabel in
            ...
            let batchTare = selectedContainer?.tareWeight_g ?? 0
            if batchTare > 0 {
                ...
                if savedTare > 0 {
                    ...
                    let container = ensureContainer()
                    container.tareWeight_g = savedTare
                } else {
                    tareWeightText = ""
                }
            }
        ...
        .onAppear {
            // Select the first container that has data, or default to "A"
        ...
        VStack(spacing: 0) {
            HStack {
                Button {
                    ...
                    HStack {
                        Text("Notes & Ratings").font(.headline).foregroundStyle(systemConfig.accent)
                        Image(systemName: isExpanded ? "chevron.up" : "chevron.down")
                            .font(.system(size: 12, weight: .semibold))
                            .foregroundStyle(Color(white: 0.45))
                        Button {
                            ...
                            Image(systemName: batch.notesLocked ? "lock.fill" : "lock.open.fill")
                                .font(.system(size: 14, weight: .medium))
                                .foregroundStyle(batch.notesLocked ? Color(red: 0.929, green: 0.278, blue: 0.290) : CMTheme.textTertiary)
                        }
                        .buttonStyle(.plain)
                        Spacer()
                    }
...
struct TareWeightHistorySheet: View {
    ...
    var body: some View {
        NavigationStack {
            Group {
                if records.isEmpty {
                    ContentUnavailableView(
                        "No History",
                        systemImage: "clock",
                        description: Text("No saved tare weights for container \(containerLabel) yet.")
                    )
                } else {
                    List {
                        ForEach(records) { record in
                            HStack {
                                VStack(alignment: .leading, spacing: 2) {
                                    Text(String(format: "%.3f g", record.weight_g))
                                        .font(.system(size: 15, design: .monospaced))
                                        .foregroundStyle(CMTheme.textPrimary)
            ...
            .navigationTitle("Container \(containerLabel) Tare History")
