...
struct SettingsView: View {
        ...
        return VStack(spacing: 0) {
            ...
            ForEach(GummyShape.allCases) { shape in
                HStack {
                    ...
                    .keyboardType(.numberPad).multilineTextAlignment(.trailing).frame(width: 70).selectAllOnFocus()
            ...
            NavigationLink {
                ...
                HStack {
                    ...
                    Text("Calibrate a Mold").font(.body).foregroundStyle(Color(red: 0.929, green: 0.278, blue: 0.290))
                    Spacer()
    ...
    private func transferLiquidsSection(systemConfig: SystemConfig) -> some View {
        @Bindable var systemConfig = systemConfig
        return VStack(spacing: 0) {
            HStack { Text("Transfer Liquids").font(.headline).foregroundStyle(CMTheme.textPrimary); Spacer() }
                .padding(.horizontal, 16).padding(.vertical, 12)
            HStack {
                ...
                Spacer()
                TextField("1.000", value: $systemConfig.lsdTransferWater_mL, format: .number.precision(.fractionLength(3)))
                    .keyboardType(.decimalPad).multilineTextAlignment(.trailing).frame(width: 60).selectAllOnFocus()
                Text("mL").font(.subheadline).foregroundStyle(CMTheme.textSecondary)
            }.padding(.horizontal, 16).padding(.vertical, 10)
        }
    ...
    private func defaultUgPerTabSection(systemConfig: SystemConfig) -> some View {
        @Bindable var systemConfig = systemConfig
        return VStack(spacing: 0) {
            HStack { Text("Default µg / Tab").font(.headline).foregroundStyle(CMTheme.textPrimary); Spacer() }
                .padding(.horizontal, 16).padding(.vertical, 12)
            HStack {
                ...
                Spacer()
                TextField("117.0", value: $systemConfig.defaultLsdUgPerTab, format: .number.precision(.fractionLength(1)))
                    .keyboardType(.decimalPad).multilineTextAlignment(.trailing).frame(width: 60).selectAllOnFocus()
        ...
        return VStack(spacing: 0) {
            ...
            if useMass {
                ...
                HStack {
                    ...
                    Spacer()
                    TextField("1063", value: sorbateMassPpmBinding, format: .number.precision(.fractionLength(0)))
                        .keyboardType(.decimalPad).multilineTextAlignment(.trailing).frame(width: 60).selectAllOnFocus()
    ...
    private func overageSection(viewModel: BatchConfigViewModel, systemConfig: SystemConfig) -> some View {
        ...
        let totalWells = Double(spec.count * viewModel.trayCount)
        ...
        return VStack(spacing: 0) {
            HStack {
                Text("Overage").font(.headline).foregroundStyle(CMTheme.textPrimary)
                Spacer(minLength: 8)
                Text(String(format: "Est. ρ %.3f g/mL", rhoMix))
                    .font(.caption).foregroundStyle(CMTheme.textSecondary)
                Button {
                    ...
                    Capsule()
                        .fill(useGummies ? systemConfig.accent.opacity(0.5) : Color.white.opacity(0.08))
                        .frame(width: 36, height: 20)
                        .overlay(alignment: useGummies ? .trailing : .leading) {
                            Circle()
                                .fill(useGummies ? systemConfig.accent : Color.white.opacity(0.35))
                                .frame(width: 16, height: 16)
                                .padding(.horizontal, 2)
                        }
                }
            ...
            HStack {
                ...
                Spacer()
                if useGummies {
                    TextField("0.0", value: gummiesBinding, format: .number.precision(.fractionLength(1)))
    ...
    private func settingsActionsSection(systemConfig: SystemConfig) -> some View {
        ...
        let hasChanges = systemConfig.hasAnySettingChanged
        return VStack(spacing: 0) {
            // Assign as defaults
            Button {
                CMHaptic.medium()
                ...
                HStack {
                    Image(systemName: "square.and.arrow.down")
                        .font(.system(size: 14, weight: .medium))
                    Text("Assign current settings as new defaults")
                        .font(.subheadline)
                    Spacer()
                }
